...
This directory contains Raspbian utility software for the Solar Pi_Platter board.
...
talkpp will echo responses from the Solar Pi_Platter to stdout.  It will also echo the last warnings or error messages that has been sent.

## ppd

ppd is a daemon for the Solar Pi_Platter.  It provides two main functions.  It will execute a controlled shutdown if the Solar Pi_Platter detects a critical battery voltage (and will power-down the entire system after [default] 30 seconds).  Since it opens the serial port associated with the Solar Pi Platter it also provides one or two mechanisms for other applications to communicate with the Solar Pi Platter.  It creates a pseudo-tty device named /dev/pi-platter which can be used just like the hardware serial port.  It also, optionally, can create a TCP port for applications like telnet to connect to.  
...
href="http://rocketblueautomation.com/pi-platter-instructions"
